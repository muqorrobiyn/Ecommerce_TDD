...
class ProductViewSetCase(APITestCase):

    def setUp(self):
        self.user = User.objects.create_user(phone_number='+998902335645', password='testcase')
        self.staff_user = User.objects.create_user(phone_number='+998993012256', password='staffcase', is_staff=True)

        self.category1 = Category.objects.create(name='meva')
        self.category2 = Category.objects.create(name='ovqat')

        self.product1 = Product.objects.create(name='behi', description='non behi', category=self.category1, price=20000)
        self.product2 = Product.objects.create(name='bosma', description="rossiyskiy kartoshka, ot go'shti, qo'y dumbasi" , category=self.category2, price=25000)
    ...
    def test_product_detail(self):
        ...
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['product']['name'], 'behi')
    ...
    def test_product_top_rated(self):
        ...
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data[0]['name'], 'behi')
    ...
    def test_permission_denied_for_anonymous_create(self):
        ...
        url = reverse('product-list')
        data = {'name': 'Test Product', 'description': 'This is Test product ', 'price': 10.00}
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
